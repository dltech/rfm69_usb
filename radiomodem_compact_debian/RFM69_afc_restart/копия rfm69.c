...
/** 
	...
	Здесь запускается событие, вызываемое через определенный промежуток времени. Оно должно будет убедиться в том что 
	превысил порог полезный сигнал, а не шум или помеха.
**/
...
/** 
	@function
	Обработчик прерывания по таймеру. Запускается через некоторый промежуток времени после превышения порога RSSI.
	Данный проомежуток соответствует времени, необходимому для успешного распознавания пакета. Если время прошло, а 
	пакет не найден, приемник перезапускается, в противном случае ничего не происходит. 
**/
void TIM2_IRQHandler(void)
{

    if( (rfm69_read(REGIRQFLAGS1) & (1<<SYNCADDRMATCH)) == 0 )  // если сигнал, превысивший порог, не был пакетом
    {
        rfm69_write(REGOPMODE, REGOPMODE_DEF | STBY_MODE);      // перезапуск приемника (бит RESTARTRX нифига не рботает, как это сказано в datasheet)
